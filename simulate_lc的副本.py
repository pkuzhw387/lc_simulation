...
class lc_simulation(object):
	...
	def read_drw_params(self):
		...
		drw = pd.read_csv(drw_file, skiprows=3, header=None, \
						  names=['log10tau', 'log10sigma_KBS09', 'Plike', 'Pnoise', 'Pinf', 'npts'],\
						  usecols=[7, 8, 14, 15, 16, 18], delim_whitespace=True)
		...
		mask = [mask1[i] and mask2[i] and mask3[i] and mask4[i] for i in range(len(self.tau_set))]
	...
	def simulate(self, output_path, cat_path=None, lc_number=None, model="DRW", dataset='SDSS_S82', prior='epoch', timescales=None, max_timescales=None):
		...
		if cat_path is None:
			...
			output_path = output_path + '_' + selected_band + '_band'
		...
		i = 0
		err_log = open(output_path + '/disgard_record', 'w')
		...
		while i < lc_number:
			...
			# if abs(MJD[-1] - MJD[0] - MJD_benchmark) < 50:
			
			try:
				print "error count is ", err_count

				if err_count >= 10:
					print "After 10 trials we still cannot generate a good light curve for this object, disgard it."
					err_log.write(str(i) + ' ' + str(self.mean_mag_set[i]) + ' ' + str(self.redshift_set[i]) + '\n')
					i += 1
					err_count = 0
					continue
				...
				if e is KeyboardInterrupt:
					sio.savemat(output_path + '/del_mat_stat.mat', del_mag_dict)
			...
			output_name = '%s/z=%f_%s_%d.txt' %(output_path, self.redshift_set[i - 1], dataset, i - 1)
